...
def main(_run, _config, _log):
    ...
    for sub_epoch in range(n_sub_epochs):
        ...
        for _, sample in enumerate(train_loader):
            ...
            query_pred, periphery_loss, align_loss, mse_loss, qry_loss ,qryputs = model(support_images, support_fg_mask,
            ...
            query_loss2 = criterion(torch.log(torch.clamp(qryputs, torch.finfo(torch.float32).eps,
                                                         1 - torch.finfo(torch.float32).eps)), query_labels)

            # bd_loss = criterion_bd(query_pred, query_labels)
            # dice_loss = criterion_dice(query_pred, query_labels)
              ##  +deve * query_loss2
            loss = query_loss + 0.1 * periphery_loss + align_loss + 0.1 * mse_loss + qry_loss +deve * query_loss2
